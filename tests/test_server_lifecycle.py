...
async def test_lifespan(mock_redis_patch: MagicMock, mock_vault_patch: MagicMock, mock_vault_config: MagicMock) -> None:
    ...
    async with app.router.lifespan_context(app):
        ...
        mock_vault_patch.return_value.authenticate.assert_awaited_once_with(
            role_id="dummy-role-id", secret_id="dummy-secret-id"
        )

    # Assert teardown
    mock_redis_patch.return_value.close.assert_awaited_once()
    mock_vault_patch.return_value.close.assert_awaited_once()
...
async def test_lifespan_vault_failure(
    ...
    mock_vault_patch.return_value.authenticate.side_effect = Exception("Vault auth failed")
...
async def test_teardown_resilience(
    ...
    mock_vault_patch.return_value.close.side_effect = Exception("Vault close error")
